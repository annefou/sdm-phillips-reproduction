...
| FORRT Outcome's `Repository URL` field | Points to `https://github.com/annefou/sdm-phillips-reproduction`. |
...
| GHCR Docker pull | `docker pull ghcr.io/annefou/sdm-phillips-reproduction:latest` over HTTPS. |
...
- [ ] Docker image builds and runs the snakemake target via `docker run --rm ghcr.io/annefou/sdm-phillips-reproduction:latest`.
